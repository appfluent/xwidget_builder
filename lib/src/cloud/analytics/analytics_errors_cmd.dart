...
class AnalyticsErrorsCommand extends BaseCommand {
  @override
  final name = 'errors';

  @override
  final description = 'Query fragment error analytics for a project';

  AnalyticsErrorsCommand() {
    argParser
      ...
      ..addOption(
        'error',
        abbr: 'e',
        help: 'Filter by error message (supports wildcards, e.g. "*parsing*")',
      )
      ..addOption(
        'range',
        abbr: 'r',
  ...
  Future<void> runAuthenticated() async {
    ...
    final errorArg = argResults!['error'] as String?;
    ...
    final loading = spinnerWorking(
      inProgressPrompt: 'Querying error events...',
      done: () => 'Errors query returned $rowCount rows.\n',
      failed: () => 'Errors query failed.\n',
    );

    try {
      final results = await api.queryErrorEvents(
        startDate: startDate,
        endDate: endDate,
        interval: interval,
      ...
      printResults(tableData, {
        ...
        'errorMessage': (label: 'Error', value: null),
        'errorCount': (label: 'Count', value: null),
      });
    } catch (e) {
      loading.failed();
      rethrow;
