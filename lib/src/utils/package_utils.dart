...
Future<String> findPackagePath(String packageName) async {
  ...
  final localPath = ConfigLoader.loadToStringOrNull(
    pubspecLock,
    "packages.$packageName.description.path",
  );
  ...
  if (!hostedDir.existsSync()) {
    throw Exception(
      "Package '$packageName' not found, because the hosted "
  ...
  if (version != null && version.trim().isNotEmpty) {
    ...
    if (!dir.existsSync()) {
      throw Exception(
        "Package '$packageName-$version' not found in "
  ...
  if (packages.isEmpty) {
    throw Exception(
      "Package '$packageName' not found in hosted packages "
