...
Future<ProcessResult> runCommand(
  ...
  try {
    return Process.run(command, arguments, workingDirectory: workingDirectory, runInShell: true);
...
String formatWithCommas(num value) {
  final parts = value.toStringAsFixed(value == value.truncate() ? 0 : 1).split('.');
...
String formatBytes(num bytes) {
  if (bytes < 1000000) {
    return "${formatWithCommas(bytes / 1000)} KB";
  }
  return "${formatWithCommas(bytes / 1000000)} MB";
}
